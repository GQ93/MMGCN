...
def run_training():
    ...
    # Forward propagation
    # build the graph
    z1= Linear.inference(input_x1=input_x2, input_dim1=d2)
    # loss function
    loss= Linear.loss(z1=z1, labels=input_labels)
    mae = Linear.MAE(labels=input_labels, logits=z1)
    # Add to the Graph the Ops that calculate and apply gradients.
    train_op = Linear.training(loss=loss, learning_rate=configMTL.learning_rate)
    ...
    for step in range(configMTL.max_steps):
        ...
        if (step + 1) % 100 == 0 or (step + 1) == configMTL.max_steps:
            ...
            print('Test Data Eval:', np.sqrt(sess.run(loss, feed_dict_test)))
            print('Test MAEs:', sess.run(mae, feed_dict_test))
