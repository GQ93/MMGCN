...
def in_class_penalty(z, sub_lap):
    ...
    """
    p = configMaskMultiGCNBrain.scale1_of_penalty * tf.linalg.trace(tf.matmul(tf.matmul(tf.transpose(z), sub_lap), z))
    return p
...
def bt_class_penalty(z1, z2, sub_laps):
    ...
    p = configMaskMultiGCNBrain.scale2_of_penalty * tf.linalg.trace(tf.matmul(tf.matmul(tf.transpose(z), sub_laps), z))
    return p
...
def inference(input_x1, input_x2, input_lap1, input_lap2,  l_sizes, input_dim1, input_dim2):
    ...
    Returns:
        logits and penalty
    """
    z1_1, z2_1, mask= ly.MaskMultiGraphConv(x1=input_x1, lap1=input_lap1, input_dim1=input_dim1, x2=input_x2,
    ...
    vis['z1'] = z1_1
    vis['z2'] = z2_1
    return logits, z1_2, z2_2, vis, mask
...
def training(loss, z1_2, z2_2, s1, s2, S, learning_rate, mask):
    """
    ...
    Mask_l1_regularizer = tf.contrib.layers.l1_regularizer(scale=configMaskMultiGCNBrain.scale_l1, scope=None)
    Mask_penalty = tf.contrib.layers.apply_regularization(Mask_l1_regularizer, [mask])
    loss += regular_in_class1 + regular_in_class2 + regular_bt_class + Mask_penalty
