...
def read_data_sets():
    ...
    S11 = configNM2TL.scale_of_Lap_within * sparse_sim(np.exp(-D1 / np.std(D1)), k=20)@sparse_sim(np.exp(-D1 / np.std(D1)), k=20)
    S12 = configNM2TL.scale_of_Lap_between * sparse_sim(np.exp(-D1 / np.std(D1)), k=20) @ sparse_sim(np.exp(-D2 / np.std(D2)), k=20)
    S21 = configNM2TL.scale_of_Lap_between * sparse_sim(np.exp(-D2 / np.std(D2)), k=20) @ sparse_sim(np.exp(-D1 / np.std(D1)), k=20)
    S22 = configNM2TL.scale_of_Lap_within * sparse_sim(np.exp(-D2 / np.std(D2)), k=20) @ sparse_sim(np.exp(-D2 / np.std(D2)), k=20)
    S = np.concatenate((np.concatenate((S11, S12), axis=1), np.concatenate((S21, S22), axis=1)), axis=0)
    print(S.shape)
    L = s2l(S, norm_state=0)
...
def placeholder_inputs(d1, d2):
    ...
    Returns:
        placeholder
    """
    input_L = tf.placeholder(tf.float32, shape=(None, None), name='L1')
    ...
    return input_L, input_x1, input_x2, input_labels


def fill_feed_dict_train(input_L, input_x1, input_x2, input_labels,
                         L, x1, x2, labels):
...
def run_training():
    ...
    print(d1, d2)
    input_L, input_x1, input_x2, input_labels = placeholder_inputs(d1, d2)
    # Forward propagation
    # build the graph
    z1, z2, r, l21w = NM2TL.inference(input_x1=input_x1, input_x2=input_x2, input_lap12=input_L, input_dim1=d1, input_dim2=d2)
    # loss function
    loss, l1, l2 = NM2TL.loss(z1=z1, z2=z2, labels=input_labels)
    mae_emoid = NM2TL.MAE(labels=input_labels, logits=z1)
    mae_nback = NM2TL.MAE(labels=input_labels, logits=z2)
    # Add to the Graph the Ops that calculate and apply gradients.
    train_op = NM2TL.training(loss=loss, r=r, l21w=l21w, learning_rate=configNM2TL.learning_rate)
    ...
    feed_dict_train = fill_feed_dict_train(input_L, input_x1, input_x2, input_labels,
                         L, data_train['emoid'], data_train['nback'], label_train)
    ...
    for step in range(configNM2TL.max_steps):
    ...
    # Write the summaries and print an overview fairly often.
        ...
        if (step + 1) % 100 == 0 or (step + 1) == configNM2TL.max_steps:
